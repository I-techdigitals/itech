      ...
      <div className="section-on-brand" style={{ padding: "40px 0" }}>
        <div className="container footer-cta">
          <div className="footer-cta-info">
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="footer-whatsapp-icon-btn" aria-label="Chat with I-TECH Digitals on WhatsApp" title="WhatsApp">
            ...
            <div>
              <div style={{ color: "rgba(255,255,255,0.9)", fontSize: "0.85rem", fontWeight: 500 }}>{t.footer.callUsFor}</div>
              <a href={`tel:+${COMPANY_PHONE_E164}`} style={{ color: "#fff", fontSize: "1.2rem", fontWeight: 700, textDecoration: "none", display: "block" }}>
      ...
      <div className="bg-brand-pattern" style={{ padding: "80px 0 40px" }}>
        <div className="container">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 40, borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: 60, marginBottom: 30 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", marginBottom: 20 }}>
                <OptimizedImage src={LOGO_SRC} alt="I-TECH Digitals" width={150} height={150} loading="lazy" sizes="150px"
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {[
                  ...
                  <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer"
                    style={{ width: 36, height: 36, background: "rgba(255,255,255,0.05)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", cursor: "pointer", transition: "all 0.3s ease", textDecoration: "none" }}
            ...
            {/* Quick Links */}
            <div>
              <h4 style={{ color: "#fff", fontSize: "1.2rem", marginBottom: 20 }}>{t.footer.quickLinks}</h4>
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                {t.footer.quickLinkLabels.map((label, i) => (
                  <li key={label}>
                    <Link href={quickLinkHrefs[i]} style={{ color: "rgba(255,255,255,0.75)", textDecoration: "none", fontSize: "0.95rem", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ color: "rgba(255,255,255,0.6)", display: "inline-flex" }}>
                        <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden>
                          <path d="M9 6l6 6-6 6" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
            ...
            {/* Contact Info */}
            <div>
              <h4 style={{ color: "#fff", fontSize: "1.2rem", marginBottom: 20 }}>{t.footer.contactInfo}</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {[
                  { label: t.footer.kuwaitOffice, addr: t.footer.kuwaitAddress },
                  { label: t.footer.pakistanOffice, addr: t.footer.pakistanAddress },
                ].map((item) => (
                  <div key={item.label} style={{ display: "flex", gap: 12 }}>
                    <div style={{ color: "rgba(255,255,255,0.7)", marginTop: 4, display: "inline-flex", flexShrink: 0 }}>
                    ...
                    <div>
                      <div style={{ color: "#fff", fontWeight: 600, fontSize: "0.95rem", marginBottom: 4 }}>{item.label}</div>
                      <div style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.75)" }}>{item.addr}</div>
                    </div>
                  </div>
                ))}
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <div>
                    <div style={{ color: "#fff", fontWeight: 600, fontSize: "0.95rem", marginBottom: 4 }}>{t.footer.emailUs}</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.9rem", flexWrap: "wrap", gap: 10, color: "rgba(255,255,255,0.75)" }}>
            <div>© {new Date().getFullYear()} I-TECH Digitals. {t.footer.allRightsReserved}</div>
